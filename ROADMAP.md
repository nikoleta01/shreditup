...
- Register sheet: stacking + no network on tap (2026-09-03) — the sheet's overlay and `BottomNav` were both `z-50`, and since `<BottomNav />` renders after `<main>` in `app/layout.tsx`, an equal z-index let the nav win the tie and paint over the bottom of the sheet: only the top sliver of the register button took taps, and only in the installed PWA where the nav sits against the home indicator. Nothing to do with the `100dvh` work — that stays. Overlay is now `z-[60]` with `pb-[calc(1rem+env(safe-area-inset-bottom))]`, and the sheet itself `max-h-[calc(100dvh-2rem)] overflow-y-auto` so a shrinking viewport (keyboard) scrolls instead of clipping. Separately, `handleRegisterTap` used to `await ensureAnonymousSession()` **before** opening the sheet: the tap sat on a network round trip with no pending state, and worse, a failed sign-in called `setError()` — which only ever renders *inside* the modal that was never opened, so the button silently did nothing. Sign-in moved into `submitRegistration`, behind the existing `submitting` spinner; the tap is now synchronous. Side benefit: accounts are created only by people who actually press submit, not by everyone who taps register and changes their mind — that behaviour is what produced the 9 profile-less `auth.users` rows (24 accounts, 15 profiles) seen before the pre-launch wipe.
